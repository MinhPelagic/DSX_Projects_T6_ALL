...
WayFinderCrl_ts WfinderCtrVar;
/*When button pressed increase/decrease*/
void HandleButton_GPSMenu_Wayfinder(DSX_OPCODE_t *newOpVal, DSX_OPCODE_t *oldOpVal)
{
    /*Check all the buttons state*/
    *newOpVal = Opcode_Matrix(
                    S509_GPS_WAYFINDER /*H_U ~ BACK*/,
                    DataDec /*L*/,
                    DataInc /*M*/,
                    *oldOpVal /*M2*/,
                    SaveAndDone /*R*/,
    ...
    else if (SaveAndDone == (*newOpVal)) // press save
    {
        ...
        {
            NewOpcode = S509_GPS_WAYFINDER;
        }
        else
        {
            /*Jump to page GOTO WAYFINDER*/
            *newOpVal = S511_GPS_WAYFINDER_GOTO;
        }
    }
